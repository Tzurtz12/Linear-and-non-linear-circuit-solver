# -*- coding: utf-8 -*-
"""
Created on Fri Jun  9 12:05:59 2023
...
def cir_reshape(cir_el, cir_nd, cir_val, cir_ctrl):
    ...
    for i in range(el_num):
        if 'a' in cir_el[i].lower():
            ...
            cir_val_r = np.append(cir_val_r, val, axis=0)
            cir_ctrl_r = np.append(cir_ctrl_r, '0')
        elif 'n' in cir_el[i].lower():
            cir_el_r = np.append(cir_el_r, cir_el[i]+"_g1")
            N1_nd = np.array([[cir_nd[i, 1], cir_nd[i, 2]]])
            cir_nd_r = np.append(cir_nd_r, N1_nd, axis=0)
            val = np.array([cir_val[i]])
            cir_val_r = np.append(cir_val_r, val, axis=0)
            cir_ctrl_r = np.append(cir_ctrl_r, '0')
            cir_el_r = np.append(cir_el_r, cir_el[i]+"_g2")
            N2_nd = np.array([[cir_nd[i, 1], cir_nd[i, 0]]])
            cir_nd_r = np.append(cir_nd_r, N2_nd, axis=0)
            cir_val_r = np.append(cir_val_r, val, axis=0)
            cir_ctrl_r = np.append(cir_ctrl_r, '0')
        else:
...
def elem_matrix(cir_el_r, cir_nd_r, cir_val_r, cir_ctrl_r, b, A, n,
    ...
    for i in range(b):
        ...
        if 'r' == cir_el_r[i][0].lower():
            ...
            M[i, i] = -1/r_pi
            N[i, i] = (beta+1)/beta
            M[i, i+1] = -beta*r0/r_pi
            N[i, i+1] = r0
            Us[i] = cir_val_r[1][0]
            Us[i+1] = cir_val_r[0][0]
    # print('M = ')
    # print(M)
    # print('N = ')
    # print(N)
